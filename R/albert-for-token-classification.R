#' Spark NLP AlbertForTokenClassification
#'
#' AlbertForTokenClassification can load ALBERT Models with a token classification head on top 
#' (a linear layer on top of the hidden-states output) e.g. for Named-Entity-Recognition (NER) tasks.
#' See \url{https://nlp.johnsnowlabs.com/docs/en/transformers#albertfortokenclassification}
#' 
...
#' 
#' @export
nlp_albert_token_classification_pretrained <- function(sc, input_cols, output_col,
  ...
  args <- list(
    ...
    validator_nlp_albert_token_classification()
  
  model_class <- "com.johnsnowlabs.nlp.annotators.classifier.dl.AlbertForTokenClassification"
...
#' @import forge
validator_nlp_albert_token_classification <- function(args) {
...
new_nlp_albertbert_for_token_classification <- function(jobj) {
  sparklyr::new_ml_transformer(jobj, class = "nlp_albert_for_token_classification")
}
