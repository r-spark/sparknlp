#' Load a pretrained Spark NLP AlbertEmbeddings model
#' 
#' Create a pretrained Spark NLP \code{AlbertEmbeddings} model
#' 
...
#' @export
nlp_albert_embeddings_pretrained <- function(sc, input_cols, output_col, case_sensitive = NULL,
  ...
  args <- list(
    ...
    validator_nlp_albert_embeddings()
  
  model_class <- "com.johnsnowlabs.nlp.embeddings.AlbertEmbeddings"
...
#' @import forge
validator_nlp_albert_embeddings <- function(args) {
...
new_nlp_albert_embeddings <- function(jobj) {
  sparklyr::new_ml_transformer(jobj, class = "nlp_albert_embeddings")
}
