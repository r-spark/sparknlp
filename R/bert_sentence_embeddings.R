#' Load a pretrained Spark NLP BertSentenceEmbeddings model
#' 
#' Create a pretrained Spark NLP \code{BertSentenceEmbeddings} model.
#' See \url{https://nlp.johnsnowlabs.com/docs/en/annotators#bertsentenceembeddings}
#' 
...
#' 
#' @export
nlp_bert_sentence_embeddings_pretrained <- function(sc, input_cols, output_col, case_sensitive = NULL,
  ...
  args <- list(
    ...
    validator_nlp_bert_sentence_embeddings()
  
  model_class <- "com.johnsnowlabs.nlp.embeddings.BertSentenceEmbeddings"
  ...
  new_nlp_bert_sentence_embeddings(model)
}

#' @import forge
validator_nlp_bert_sentence_embeddings <- function(args) {
...
new_nlp_bert_sentence_embeddings <- function(jobj) {
  sparklyr::new_ml_transformer(jobj, class = "nlp_bert_sentence_embeddings")
}
