...
test_that("ner_converter param setting", {
  ...
  test_param_setting(sc, nlp_ner_converter_internal, test_args)
})
...
test_that("nlp_ner_converter_internal ml_pipeline", {
  test_annotator <- nlp_ner_converter_internal(pipeline, input_cols = c("document","token","ner"), output_col = "ner_span")
  transformed_data <- ml_fit_and_transform(test_annotator, text_tbl)
  expect_true("ner_span" %in% colnames(transformed_data))
})

test_that("nlp_ner_converter_internal tbl_spark", {
  transformed_data <- nlp_ner_converter_internal(test_data, input_cols = c("document","token","ner"), output_col = "ner_span")
  expect_true("ner_span" %in% colnames(transformed_data))
})
