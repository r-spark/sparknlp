...
test_that("nlp_xlnet_token_classification pretrained", {
  model <- nlp_xlnet_token_classification_pretrained(sc, input_cols = c("sentence", "token"), output_col = "bert")
  transformed_data <- ml_transform(model, test_data)
  expect_true("bert" %in% colnames(transformed_data))
})

test_that("nlp_xlnet_token_classification load", {
  model_files <- list.files("~/cache_pretrained/")
  bert_model_file <- max(Filter(function(s) startsWith(s, "xlnet_base_token"), model_files))
