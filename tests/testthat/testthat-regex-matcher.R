...
test_that("nlp_regex_matcher ml_pipeline", {
  test_annotator <- nlp_regex_matcher(pipeline, input_cols = c("document"), output_col = "regex", rules_path = here::here("tests", "testthat", "data", "regex_match.txt"))
  transformed_data <- ml_fit_and_transform(test_annotator, test_data)
  expect_true("regex" %in% colnames(transformed_data))
})

test_that("nlp_regex_matcher tbl_spark", {
  transformed_data <- nlp_regex_matcher(test_data, input_cols = c("document"), output_col = "regex", rules_path = here::here("tests", "testthat", "data", "regex_match.txt"))
  expect_true("regex" %in% colnames(transformed_data))
})
